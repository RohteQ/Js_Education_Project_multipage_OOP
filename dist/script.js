...
window.addEventListener('DOMContentLoaded', () => {
  ...
  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_2__["default"]('.module__video-item .play', '.overlay').init(); //alt call
...
class VideoPlayer {
  constructor(triggers, overlay) {
    ...
    this.close = this.overlay.querySelector('.close'); //cause we won't lost context of call this  str 54
    ...
    this.btns.forEach((btn, i) => {
      ...
      btn.addEventListener('click', () => {
        ...
        if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== "true") {
          this.activeBtn = btn; //only when player called already,iframe with id frame replace div with id frame. if player already exist => only show overlay

          if (document.querySelector('iframe#frame')) {
            this.overlay.style.display = 'flex'; //check if user click on another btn
            ...
            this.path = btn.getAttribute('data-url'); //we dont need to create new player every time, using youtube API loadVideoById
  ...
  onPlayerStateChange(state) {
    try {
      ...
      const blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; //get svg icon from not blocked btn, true=> deep clone

      const playBtn = this.activeBtn.querySelector('svg').cloneNode(true); //event.data -> in utube API, 0 = when video ended

      if (state.data === 0) {
        //if element blocked
        if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
          blockedElem.querySelector('.play__circle').classList.remove('closed'); //removing blocked svg icon
          ...
          blockedElem.style.filter = 'none'; //if video ended set atribute data disabled in false
...
class MainSlider extends _slider__WEBPACK_IMPORTED_MODULE_1__["default"] {
  ...
  showSlides(n) {
    ...
    } catch (e) {} //hide all elements, if we use this.slides without ... - not working,cause children return Pseudo-array without forEach method
  ...
  bindTriggers() {
    this.btns.forEach(btn => {
      ...
      btn.parentNode.previousElementSibling.addEventListener('click', e => {
        e.preventDefault();
        this.slideIndex = 1; // repeat initialization
